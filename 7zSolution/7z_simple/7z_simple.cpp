...
struct scoped_currentdirectory
{
	...
	~scoped_currentdirectory()
	{
		::SetCurrentDirectory(origin_current_dir);
	}

	TCHAR	origin_current_dir[MAX_PATH];
};
...
#ifdef _WIN32
	{
			...
			return SZ_ERROR_MEM;
		{
			...
			int numChars = WideCharToMultiByte(fileMode ?
				...
				0, reinterpret_cast<LPCWSTR>(s), len, (char *)buf->data, size, &defaultChar, &defUsed);
...
static WRes MyCreateDir(const UInt16 *name)
{
	...
	return CreateDirectoryW(reinterpret_cast<LPCWSTR>(name), NULL) ? 0 : GetLastError();
...
{
#ifdef USE_WINDOWS_FILE
	return OutFile_OpenW(p, reinterpret_cast<LPCWSTR>(name));
#else
	CBuf buf;
	WRes res;
	Buf_Init(&buf);
	...
	if (res == SZ_OK)
	{
		...
		*/
		...
		for (i = 0; i < db.db.NumFiles; i++)
		{
			...
			#ifdef USE_WINDOWS_FILE
			if (f->AttribDefined)
				SetFileAttributesW(reinterpret_cast<LPCWSTR>(destPath), f->Attrib);
			#endif
