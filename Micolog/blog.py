...
class MainPage(BasePublicPage):
	def get(self,page=1):
		postid=self.param('p')
		ajax=self.param('ajax')
		if postid:
			try:
				postid=int(postid)
				return doRequestHandle(self,SinglePost(),postid=postid,ajax=ajax)  #singlepost.get(postid=postid)
			except:
...
class entriesByCategory(BasePublicPage):
	...
	def get(self,slug=None):
		...
		if cats:
			...
			self.render(templete,{'entries':entries,'category':cats[0],'pager':links})
		else:
			self.error(414,slug)
...
class archive_by_month(BasePublicPage):
	...
	def get(self,year,month):
		...
		entries,links=Pager(query=entries).fetch(page_index)
                ...
                if(ajax):
                    templete='month_ajax';
		self.render(templete,{'entries':entries,'year':year,'month':month,'pager':links})
...
class entriesByTag(BasePublicPage):
	...
	def get(self,slug=None):
		...
		entries,links=Pager(query=entries,items_per_page=20).fetch(page_index)
		self.render(templete,{'entries':entries,'tag':slug,'pager':links})
...
class SinglePost(BasePublicPage):
	...
	@cache()
	def get(self,slug=None,postid=None,ajax=None):
		if postid:
...
##		else:
		...
		comments_nav=self.get_comments_nav(mp,entry.purecomments().count())
                ...
                if(ajax):
                    templete=templete+'_ajax'

                self.render(templete,
                                {
                                'entry':entry,
